class MetaService {
	private META_KEYWORD = `Levée de fonds`;
	...
	private filterMetaDescription() {
		...
		if (currentFundraiser?.appeal) {
			...
			meta_description = `${appeal_string} | ${this.META_KEYWORD}`;
		...
		if (currentFundraiser?.content) {
			...
			meta_description = `${content_string} | ${this.META_KEYWORD}`;
			return meta_description;
		}
	...
	private isFundraiserIndexable() {
		...
		if (this.fundraiserApiRes?.data?.is_draft === true) return false;
		if (this.fundraiserApiRes?.data?.is_findable === false) return false;
		if (this.fundraiserApiRes?.data?.deleted === true) return false;
		if (this.fundraiserApiRes?.data?.live === false) return false;
		return true;
	}
...
const fundraisingRequestHandler: PagesFunction = async (context) => {
	try {
		...
		const getFundraiserAPI = fetch(
			`${API_ENDPOINT}fundraiser/get?slug=${slug}&language=fr`
